...
export function CartSidebar() {
    const { items, removeItem, updateQuantity, totalPrice, totalItems, isOpen, closeCart, toggleCart } = useCart();
    ...
    // Bloqueia o scroll quando aberto
    useEffect(() => {
        if (isOpen) {
    ...
    return (
        <>
            ...
            <div
                className={`fixed inset-0 bg-black/40 backdrop-blur-[2px] z-[9998] transition-opacity duration-300 ${isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
                onClick={closeCart}
                aria-hidden="true"
            />

            {/* --- DRAWER (CARRINHO LATERAL) --- */}
            <div className={`fixed right-0 top-0 h-full w-full sm:w-[450px] bg-white z-[9999] shadow-2xl transform transition-transform duration-300 ease-out flex flex-col ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
                ...
                <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-white z-10">
                    ...
                    <button onClick={closeCart} className="p-2 hover:bg-slate-100 rounded-full transition-colors text-slate-500 hover:text-red-500">
                        <X className="w-6 h-6" />
                    </button>
                </div>
                ...
                {items.length > 0 && (
                    <div className="bg-white border-t border-slate-100 p-6 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-10">
                        ...
                        <Link href="/pagamento" onClick={closeCart} className="block">
